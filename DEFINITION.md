...
- ~~PostgreSQL como almacenamiento primario o por defecto — libSQL es el motor primario; Postgres es un destino de exportación bajo demanda, no un requisito de v1.~~ **SUPERADO por CONTRACT-19/20/21 (2026-07-25): `librarian` ARRANCA Y SIRVE sobre PostgreSQL**, elegido por configuración (`LIBRARIAN_ENGINE`). libSQL sigue siendo el motor por defecto y el de menor costo operativo, pero Postgres dejó de ser solo un destino de exportación. El texto original se conserva porque haberlo escrito así fue un error de encuadre que vale registrar: no era una capacidad postergable, era **la premisa** que le da sentido a construir sobre `sqlite-postgres-compat`. Listarla como "fuera de alcance" la dejó sin verificar durante meses, mientras la exportación —que sí funcionaba— ocupaba el lugar mental de "está probado". Ver `docs/reports/CONTRACT-21-REPORT.md`.
